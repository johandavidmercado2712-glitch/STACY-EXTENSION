...
function fmtDate(d) {
  if (!d) return "";
  const dt = new Date(
    typeof d === "string" && d.includes(" ") ? d.replace(" ", "T") : d,
  );
  return isNaN(dt)
    ? d
...
function cDate(c) {
  return c.fecha || c.COM_FECHA || c.fecha_ejecucion || c.date || "";
}
function cDir(c) {
  return c.ruta || c.COM_RUTA || c.directorio || c.directory || "";
}
function fName(f) {
  return (
    f.CAR_NOMBRE ||
...
function fId(f) {
  return f.CAR_ID || f.id || f.car_id || 0;
}
  ...
  return !notes || !notes.length
    ...
    : notes
        .map(function (n) {
          ...
          const preview = (n.NOT_CONTENIDO || n.contenido || n.content || "")
            .replace(/\n/g, " ")
            .substring(0, 80);
          const date = shortDate(
            n.NOT_UPDATED_AT || n.updated_at || n.NOT_CREATED_AT || "",
          );
  ...
  <div class="modal-box" style="max-width:440px">
    ...
    <div class="modal-body">
      <div class="os-selector">
        ...
        <button class="os-tab" data-os="linux">Linux / Mac</button>
      </div>
...
class StacyWebViewProvider {
  ...
  resolveWebviewView(v) {
    ...
    console.log(
      "[STACY] resolveWebviewView: setting webview html length=" +
    ...
    v.webview.onDidReceiveMessage(async (msg) => {
      ...
      if (msg.type === "ready") {
        this._ready = true;
        console.log(
          '[STACY] webview ready, _update will be called, _user="' +
            this._user +
            '"',
        );
        this._update();
      ...
      switch (msg.type) {
        ...
        case "logout":
          ...
          break;
        case "import":
          vscode.commands.executeCommand("stacy.importCommand");
          break;
        case "googleLogin":
          vscode.commands.executeCommand("stacy.googleLogin");
          break;

        case "login":
          console.log(
            "[STACY] login msg received, _ready=" +
              this._ready +
              ' _user="' +
              this._user +
              '"',
          ...
          if (this._auth && this._ctx) {
            try {
              ...
              this._loginErr = "";
              console.log(
                "[STACY] login OK, calling _update, _ready=" +
                  this._ready +
                  ' _user="' +
                  this._user +
                  '"',
              ...
              try {
                ...
                console.error("[STACY] _update after login failed", e);
              }
        ...
        case "createNote":
          try {
            await this._client.createNote(msg.title, msg.content);
            vscode.commands.executeCommand("stacy.refresh");
          } catch (err) {
        ...
        case "deleteNote":
          try {
            await this._client.deleteNote(msg.id);
            vscode.commands.executeCommand("stacy.refresh");
          } catch (err) {
            vscode.window.showErrorMessage("Error: " + err.message);
          }
...
if (Test-Path \$histFile) {
  \$allLines = @(Get-Content \$histFile -Encoding UTF8)
  \$lines = @()
  
  foreach (\$line in \$allLines) {
    \$trimmed = \$line.Trim()
    if (\$trimmed -and \$trimmed.Length -gt 0 -and \$trimmed -notmatch "[\r\n]" -and \$trimmed.Length -lt 1000) {
      \$lines += \$trimmed
    }
  }
  
  \$total = @(\$lines).Count
  Write-Host "Encontrados \$total comandos validos (de \$(\$allLines.Count) en historial)"
  
  if (\$total -gt 0) {
    Write-Host "Leyendo \$total comandos..." -ForegroundColor Green
    \$importados = 0
    \$idx = 0
    \$batchNum = 0
    \$rechazados = 0
    ...
    while (\$idx -lt \$total) {
      ...
      for (\$j = \$idx; \$j -lt \$end; \$j++) {
        ...
        \$batch += @{ 
          comando = [string]\$cmd
          directorio = "[MAQUINA:\$HOSTNAME] \$env:USERPROFILE"
        }
      }
      
      \$idx = \$end
      Write-Host "Lote \$batchNum - Enviando \$batchSize comandos..."
      
      \$payload = @{ comandos = \$batch }
      \$body = ConvertTo-Json -InputObject \$payload -Depth 3
      
      Write-Host "  JSON enviado (primeros 200 caracteres): \$(\$body.Substring(0, [Math]::Min(200, \$body.Length)))" -ForegroundColor DarkGray
      
      \$retryCount = 0
      \$success = \$false
      
      while (\$retryCount -lt 2 -and -not \$success) {
        \$retryCount++
        try {
          \$bodyBytes = [System.Text.Encoding]::UTF8.GetBytes(\$body)
          \$resp = Invoke-RestMethod -Uri "\$API_URL/comandos/importar" \`
            -Method Post \`
            -Body \$bodyBytes \`
          ...
          Write-Host "  Respuesta: \$(\$resp | ConvertTo-Json)" -ForegroundColor Gray
          
          if (\$resp) {
            \$count = 0
            if (\$resp.importados -ne \$null) {
              \$count = \$resp.importados
            } elseif (\$resp.Count -is [int]) {
              \$count = \$resp.Count
            } elseif (\$resp -is [int]) {
              \$count = \$resp
            }
            ...
            if (\$count -gt 0) {
              \$importados += \$count
              Write-Host "  OK: +\$count (total: \$importados/\$total)" -ForegroundColor Green
              \$success = \$true
            } else {
              \$rechazados += \$batchSize
              Write-Host "  NOTA: No se importaron comandos (ya registrados en STACY)" -ForegroundColor Yellow
              \$success = \$true
            }
          } else {
            Write-Host "  ERROR: respuesta nula del servidor"
            if (\$retryCount -eq 1) {
          ...
          if (\$retryCount -eq 1) {
            ...
            Write-Host "  FALLO FINAL: \$errMsg" -ForegroundColor Red
          }
        }
      }
    }
    Write-Host "Completo: \$importados/\$total comandos importados (rechazados: \$rechazados)" -ForegroundColor Cyan
  } else {
    Write-Host "Historial vacio" -ForegroundColor Yellow
  }
} else {
  Write-Host "Archivo no encontrado: \$histFile" -ForegroundColor Red
}
...
' "$API_URL" "$TOKEN" "$HOSTNAME"`;
            ...
            if (open === "Ejecutar ahora") {
              ...
              if (isWin && isWSL) {
                ...
                term.show();
                term.sendText(
                  '[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; & "' +
                    winPath +
                    '"',
                  true,
                );
                ...
                term.show();
                term.sendText(
                  'powershell -ExecutionPolicy Bypass -File "' + safePath + '"',
                  true,
                );
              } else {
                const term = vscode.window.createTerminal("STACY Upload");
        ...
        case "importWindowsHistory":
          try {
            ...
            if (isWSL) {
              ...
              if (!winUser) {
                vscode.window.showErrorMessage(
                  "No se pudo detectar el usuario de Windows",
                );
                break;
              }
              histPath =
            ...
            if (!fs.existsSync(histPath)) {
              vscode.window.showErrorMessage(
                "No se encontro el historial de PowerShell en:\n" +
                  histPath +
            ...
            if (lines.length === 0) {
              vscode.window.showErrorMessage(
                "El archivo de historial esta vacio",
              );
              break;
            }
            const userName = isWSL
              ? "RED5G"
            ...
            vscode.window.withProgress(
              ...
              async () => {
                const coms = lines.map((l) => ({
                  comando: l,
                  directorio:
                    "[MAQUINA:" + userName + "-WIN] C:\\Users\\" + userName,
                }));
                const batchSize = 100;
                let imported = 0;
                ...
                vscode.window.showInformationMessage(
                  imported + " comandos importados desde PowerShell",
                );
                vscode.commands.executeCommand("stacy.refresh");
              },
            );
            ...
            vscode.window.showErrorMessage(
              "Error al importar historial: " + err.message,
            );
          }
          break;
        ...
        case "assignCommand":
          try {
            await this._client.assignCommandToFolder(
              msg.comandoId,
              msg.carpetaId,
            );
            vscode.commands.executeCommand("stacy.refresh");
        ...
        case "saveCommandDescription":
          try {
            await this._client.updateCommandDescription(
              msg.carpetaId,
              msg.comandoId,
              msg.descripcion,
            );
  ...
  _update() {
    if (!this._view) {
      console.log("[STACY] _update skipped: no _view");
      return;
    }
    if (!this._ready) {
      console.log(
        '[STACY] _update skipped: _ready=false, user="' +
          (this._user || "") +
          '"',
      );
      return;
    ...
    console.log(
      "[STACY] _update posting setData loggedIn=" +
        loggedIn +
        " cmds=" +
        (this._ac || []).length,
    );
